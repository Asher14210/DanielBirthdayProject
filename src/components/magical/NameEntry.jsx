...
export default function NameEntry({onContinue,busy=false,error=''}) {
  const [name,setName]=useState('');
  const submit=()=>{ if(name.trim()&&!busy) onContinue(name.trim()); };
  return <motion.section className="name-scene" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} transition={{duration:.6}}>
    <motion.div className="name-slip" initial={{y:-40,rotate:-2,opacity:0}} animate={{y:0,rotate:-1,opacity:1}} transition={{duration:.8,ease:[.16,.8,.2,1]}}>
      ...
      <input className="name-input" type={name.length>32?'password':'text'} value={name} onChange={e=>setName(e.target.value)}
        onKeyDown={e=>e.key==='Enter'&&submit()} placeholder="Your name" maxLength={256} aria-label="Your name" autoComplete="off" disabled={busy}/>
      <PaperButton onClick={submit} disabled={busy}>{busy?'Please wait…':'Seal & begin'}</PaperButton>
