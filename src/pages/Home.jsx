...
const newTimer=()=>({started:false,activeSince:null,elapsed:0,submitted:false});
...
export default function Home(){
 ...
 useEffect(()=>{let active=true;tracker('status').then(status=>{if(!active)return;setAdmin(status.admin);if(status.admin&&new URLSearchParams(window.location.search).get('test')==='1'){setName('Admin');setScene('arrival')}else if(status.submitted)setError('This browser has already chosen a letter.')}).catch(e=>{if(active)setError(e.message)}).finally(()=>{if(active)setBusy(false)});return()=>{active=false}},[]);
 useEffect(()=>{const esc=e=>e.key==='Escape'&&scene==='reading'&&closeLetter();window.addEventListener('keydown',esc);return()=>window.removeEventListener('keydown',esc)},[scene,admin]);
 ...
 const begin=async n=>{
  ...
  try{
   ...
   if(status.submitted&&!status.admin)throw new Error('This browser has already chosen a letter.');
   setName(n);timing.current=newTimer();setScene('arrival');
  }catch(e){setError(e.message)}finally{pending.current=false;setBusy(false)}
 };
 const closeLetter=()=>{setRead(s=>new Set(s).add(letter.id));if(admin){timing.current=newTimer();setScene('browse')}else{setError('Your letter has been saved. This browser can only choose once.');setScene('name')}};
 const choose=()=>{if(!pending.current){setError('');setScene('confirm')}};
 const openLetter=async()=>{
  ...
  try{await tracker('',{name,letterId:letter.id,durationMs:Math.min(86_400_000,Math.round(timer.elapsed)),adminTest:admin});setScene('opening')}
  catch(e){timer.submitted=false;timer.activeSince=document.hidden?null:performance.now();setError(e.message);if(e.status===409)setScene('name')}
  finally{pending.current=false;setBusy(false)}
 };
 const resetTest=()=>{if(!pending.current){timing.current=newTimer();setError('');setScene('arrival')}};
 return <DeskScene close={scene==='opening'||scene==='reading'}>
  ...
  {scene==='confirm'&&<><LetterCarousel key="under" letters={letters} selected={selected} setSelected={setSelected} read={read} onChoose={choose}/><ConfirmationSlip key="confirm" letter={letter} first={!admin} busy={busy} error={error} onYes={openLetter} onNo={()=>setScene('browse')}/></>}
